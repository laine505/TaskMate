 ...
 * @prop {string}   [posterAvatarUrl]– poster's profile photo URL
 * @prop {function} onPress
 */
export default function GigCard({ title, author, rating, delivery, price, imageUrl, posterAvatarUrl, onPress }) {
  const resolvedUri = imageUrl || getFallbackImage(title);
  const isNew = rating === "New";
  // Extract just the name part (before " · ") for the avatar initial
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.88}>
      ...
      <View style={styles.content}>
        {/* Poster row with avatar */}
        <View style={styles.authorRow}>
          {posterAvatarUrl ? (
            <Image source={{ uri: posterAvatarUrl }} style={styles.authorAvatar} resizeMode="cover" />
          ) : (
...
const styles = StyleSheet.create({
  ...
  authorAvatarText: { color: "white", fontSize: 11, fontWeight: "700" },
  author:   { color: "#6b7280", fontSize: 12, flex: 1 },
